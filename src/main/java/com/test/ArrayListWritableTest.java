...
public class ArrayListWritableTest {
	...
	/*
	 * @Test public void testTypeSafety() {
	 * ArrayListWritable<WritableComparable> list = new
	 * ArrayListWritable<WritableComparable> (); list.add(new Text("Hello"));
	 * list.add(new Text("Are you there"));
	 * 
	 * try { list.add(new IntWritable(5)); assertTrue(false); // should throw an
	 * exception before reaching this line. } catch (IllegalArgumentException e)
	 * { assertTrue(true); }
	 * 
	 * ArrayList<WritableComparable> otherList = new
	 * ArrayList<WritableComparable>(); otherList.add(new Text("Test"));
	 * otherList.add(new Text("Test 2"));
	 * 
	 * assertTrue(list.addAll(otherList));
	 * 
	 * otherList.add(new IntWritable(6)); try { list.addAll(otherList);
